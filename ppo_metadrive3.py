...
class NNPolicy:
    ...
    def __init__(self, net: Actor, vae: Optional[ImageVAE_CNN840Micro_LinearDec] = None, use_recon_for_policy: bool = True):
        self.net = net
        self.vae = vae
    ...
    def __call__(self, obs: npt.NDArray) -> Tuple[float, float]:
        ...
        # z / recon 생성
        with torch.no_grad():
        # If your VAE now predicts actions, DO NOT use it as x_in.
...
class PPOTrainer:
    ...
    def save_model(self, path: str):
        ...
        torch.save({
            ...
            'vae_total_losses': self.vae_total_losses,                       # <-- 추가
            'vae_recon_losses': self.vae_recon_losses,                       # <-- 추가
    ...
    def load_model(self, path: str):
        ...
        self.vae_optimizer.load_state_dict(checkpoint['vae_optimizer_state_dict'])  # <-- 추가
